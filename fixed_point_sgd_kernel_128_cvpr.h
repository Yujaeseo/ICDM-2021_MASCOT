...
__forceinline__  __device__ float comp_grad_int8_cvpr(char f_val1, char f_val2, float scaling_factor ,char4 int_val2){
    char4 int_val1 = make_char4(f_val1,
                                f_val2,
                                0,
                                0);
...
__global__ void sgd_k128_kernel_hogwild_warp32_lrate_mean_diff_adaptive_fixed_point(
                            const Node *R,
                            unsigned int nnz,
                            float* p,
                            float* q,
...
__global__ void sgd_k128_kernel_hogwild_warp32_lrate_mean_diff_adaptive_fixed_point_log(
                            const Node *R,
                            unsigned int nnz,
                            float* p,
                            float* q,
                            ...
                            int update_vector_size,
                            float lambda_f,
                            int* count_p_bitwidth,
                            int* count_q_bitwidth,
                            int* count_backward_bitwidth
                            )
{
    int lane_id = threadIdx.x%32;
    int local_wid = threadIdx.x/32;
    ...
    {    
        ...
        for(int i = 0;i < update_vector_size;i++)
        {
            ...
            // //! Remove
            if (user_quantization_params.n == 8){
                count_p_bitwidth[wid] += 1;
            }else{
                count_p_bitwidth[2048 + wid] += 1;
            }

            if (item_quantization_params.n == 8){
                count_q_bitwidth[wid] += 1;
            }else{
                count_q_bitwidth[2048 + wid] += 1;
            }
            // //! 
            ...
            // //! Remove
            if (back_ward_quantization_params.n == 8){
                count_backward_bitwidth[wid] += 1;
            }else{
                count_backward_bitwidth[2048 + wid] += 1;
            }
            // //! 
